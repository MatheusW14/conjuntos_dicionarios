"""
Marina trabalha no setor de segurança de uma empresa e precisa verificar
se um determinado conjunto de permissões faz parte das permissões principais de um sistema.
Sua tarefa é desenvolver um programa que receba duas listas de permissões e
...
permissoes_solicitadas = set(
    input("Permissões solicitadas: ").strip().lower().split(",")
)
